...
export function buildManualEditEvidence({ cwd = process.cwd(), pageUrl = null } = {}) {
  ...
  return {
    ...
    context: {
      cwd,
      bufferPath: toProjectRelative(cwd, getBufferPath(cwd)),
...
function analyzeSourceHint(op, cwd) {
  ...
  const relativeFile = toProjectRelative(cwd, file);
...
function maybeAddSearchFile(file, cwd, seenFiles, out) {
  ...
  out.push({ file, relativeFile: toProjectRelative(cwd, file), content, lines: content.split('\n') });
}
