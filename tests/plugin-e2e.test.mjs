 ...
 * workflow regenerates on main (`npm run build:release`). Between the launcher
 ...
 * was verified against a local `npm run build:release` before the swap
 * merged), and tests/hook-build.test.mjs gates its launcher-path assertions on
 * the synced output.
 *
 * Run with: npm run test:plugin-e2e
 */
